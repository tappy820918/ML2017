...
def vis_img_in_filter(img,layer_dict,model,
    ...
    ax[0, 0].imshow(img.reshape((48, 48)), cmap='rainbow')
    ...
    for (x, y) in [(i, j) for i in range(plot_x) for j in range(plot_y)]:
        if x == 0 and y == 0:
            continue
        ax[x, y].imshow(img_ascs[x * plot_y + y - 1], cmap='rainbow')
...
def main():
    ...
    #private_pixels = np.load('./feature/X_train.npy')
    #print(private_pixels.shape)
    private_pixels = [ np.fromstring(X[i], dtype=float, sep=' ').reshape((1, 48, 48, 1)) for i in range(len(X)) ]
    vis_img_in_filter(private_pixels[24],layer_dict,emotion_classifier)
